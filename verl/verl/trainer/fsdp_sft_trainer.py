...
class FSDPSFTTrainer:
    def __init__(self, config, device_mesh: DeviceMesh, ulysses_device_mesh: DeviceMesh):
        ...
        self.processor = hf_processor(local_model_path, use_fast=True)  # used for multimodal LLM, could be none
    ...
    def _build_dataloader(self):
        ...
        self.train_dataset = dataset_cls(parquet_files=config.data.train_files, tokenizer=self.tokenizer, config=config.data, processor=self.processor)
        self.val_dataset = dataset_cls(parquet_files=config.data.val_files, tokenizer=self.tokenizer, config=config.data, processor=self.processor)
        ...
        if self.processor is not None:
            ...
            with context, torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                if not use_sp:
                    ...
                    # Not implemented yet
                    raise NotImplementedError("Sequence parallel with processor is not implemented yet")
        else:
            with context, torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                if not use_sp:
                    # Standard forward pass without sequence parallel
